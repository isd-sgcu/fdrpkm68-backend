...
export class RpkmRepository {
  ...
  async getWorkshopsRegisteredByUserID(
    userId: string
  ): Promise<RPKMworkshop[]> {
    try {
      ...
      console.log(
        `Error getting RPKM workshops registered by user id ${userId}: `,
        error
      );
      throw new Error(
  ...
  async getWorkshopsParticipantCounts(): Promise<
    workshopParticipantCountType[]
  > {
    try {
      const allTypes = Object.values(WorkshopType);
      const baseCounts: Record<WorkshopType, number> = {} as Record<
        WorkshopType,
        number
      >;
      ...
      console.log(
        `Error getting participant counts of RPKM workshops: `,
        error
      );
  ...
  async getUserWorkshopConflicts(
    body: WorkshopRegisterRequest,
    userId: string
  ): Promise<RPKMworkshop[][]> {
    try {
      ...
      console.log(
        `Error checking workshop time/type conflicts of user id ${userId}: `,
        error
      );
      throw new Error(
        `Failed to check workshop time/type conflicts of user id ${userId}.`
      );
    }
  }
}
